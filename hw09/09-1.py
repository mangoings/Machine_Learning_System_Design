...
simpleRNN_model = Sequential([
    SimpleRNN(units=n_units, return_sequences=False, input_shape=(seq_length-1, 1)),
    Dense(1)
])

simpleRNN_model.compile(optimizer='adam', loss='mse')

history = simpleRNN_model.fit(X_train, y_train, epochs=epochs, validation_data=(X_test, y_test), verbose=0)

train_predict = simpleRNN_model.predict(X_train)
test_predict = simpleRNN_model.predict(X_test)
